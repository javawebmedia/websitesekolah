...
listSiswa();		
var table = $('#siswaListing').dataTable({
	"searching": true,
  "ordering": true
}); 
...
function listSiswa(){
	$.ajax({
		type  : 'GET',
		url   : '<?php echo base_url("admin/rombel/siswa_rombel/".$rombel->id_rombel) ?>',
		async : false,
		dataType : 'json',
		success : function(data){
			...
			for(i=0; i<data.length; i++){
				html += '<tr id="'+data[i].id_siswa_rombel+'">'+
						'<td class="text-center">'+data[i].no_urut+'</td>'+
						'<td>'+data[i].nama_siswa+'</td>'+
						'<td>'+data[i].jenis_kelamin+'</td>'+
						'<td>'+data[i].ttl+'</td>'+	
						'<td>'+
							'<a href="javascript:void(0);" class="btn btn-light btn-sm deleteRecord" data-id="'+data[i].id_siswa_rombel+'" title="Hapus"><i class="fa fa-trash"></i></a>'+
						'</td>'+
						'</tr>';
			}
...
$('#simpan').submit('click',function(){
	...
	var status_siswa_rombel = $('#status_siswa_rombel').val();
	$.ajax({
		type : "POST",
		url  : "<?php echo base_url('admin/rombel/tambah_siswa') ?>",
		dataType 	: "JSON",
		data 			: { 
									id_user: id_user, 
									id_siswa:id_siswa,
									...
									id_tahun:id_tahun,
									id_tahun:id_tahun,
									status_siswa_rombel: status_siswa_rombel
								},
		success: function(data){
			$('#id_siswa').val("");
			listSiswa();
	...
	// delete rombel record
	 $('#deleteEmpForm').on('submit',function(){
		...
		$.ajax({
			...
			dataType : "JSON",  
			data : { id_siswa_rombel:pesertaId },
			success: function(data){
				$("#"+pesertaId).remove();
				$('#deleteEmpId').val("");
